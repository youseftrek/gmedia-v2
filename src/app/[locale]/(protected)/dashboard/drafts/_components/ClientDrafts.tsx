...
const ClientDrafts = ({ session, locale }: Props) => {
  ...
  useEffect(() => {
    async function fetchDrafts() {
      setIsLoading(true);
      try {
        const res = await getDrafts(session, currentPage, itemsPerPage, locale);
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-2">
          ...
          {drafts.map((draft: any) => (
            <RequestCard key={draft.id} request={draft} page="drafts" />
          ))}
        </div>
      )}
