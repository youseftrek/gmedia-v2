...
const ClientNeedActions = ({ session, locale }: Props) => {
  ...
  const t = useTranslations("NeedActionsPage");
  ...
  useEffect(() => {
    async function fetchDrafts() {
      setIsLoading(true);
      try {
        const res = await getRequestsNeedActions(
          session,
          currentPage,
          itemsPerPage,
          locale
      ...
      <div className="flex flex-col gap-2">
        ...
        {isLoading ? (
          ...
          drafts.map((draft: any) => (
            <RequestCard key={draft.id} request={draft} page="need-actions" />
          ))
        )}
      </div>
...
export default ClientNeedActions;
